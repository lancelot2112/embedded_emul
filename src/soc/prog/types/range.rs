...
impl fmt::Display for RangeSpecError {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			...
			RangeSpecError::LengthLiteral(err) => write!(f, "failed to parse length literal: {err}"),
			RangeSpecError::InvalidOrdering { start, end } => write!(f, "end {end:#x} precedes start {start:#x}"),
			...
			RangeSpecError::BitLengthNotByteAligned { bits } => write!(f, "bit length {bits} is not byte aligned"),
			RangeSpecError::ExtraTokens(tok) => write!(f, "unexpected tokens '{tok}' in range length"),
		}
	}
}
	...
	match parse_u64_literal(literal_token) {
		...
		Err(err) => {
			...
			if let Some((literal, suffix)) = strip_appended_suffix(literal_token) {
				let value = parse_u64_literal(literal)
					.map_err(RangeSpecError::LengthLiteral)?;
				let unit = parse_unit(suffix)
					.ok_or_else(|| RangeSpecError::UnknownUnit(suffix.to_string()))?;
				apply_unit(value, unit)
			} else {
...
mod tests {
	...
	fn rejects_misaligned_bits() {
		...
		assert!(matches!(err, RangeSpecError::BitLengthNotByteAligned { .. }));
	}
